...
function gpgEn()
    ...
    local newTxt, err = shell.RunCommand("fcifra -e " .. "'" .. oldTxt .. "'")
...
function gpgDe()
    ...
    local a, b = getTextLoc()
    local oldTxt = getText(a,b)
    local newTxt, err = shell.RunCommand("fcifra -d " .. "'" .. oldTxt .. "'")
...
function init()
    config.MakeCommand("gpgEn", gpgEn, config.NoComplete)
    config.MakeCommand("gpgDe", gpgDe, config.NoComplete)
end
